...
def Q5Saver(ConvW1, ConvB1, ConvW2, ConvB2, ConvW3, ConvB3, FC_w1, FC_b1, FC_w2, FC_b2, trainPhase, sess):
    ...
    np.savetxt(questionNum+phase+'FC_w1.out', FC_w1.eval(session = sess))
    np.savetxt(questionNum+phase+'FC_b1.out', FC_b1.eval(session = sess))
    np.savetxt(questionNum+phase+'FC_w2.out', FC_w2.eval(session = sess))
    np.savetxt(questionNum+phase+'FC_b2.out', FC_b2.eval(session = sess))
...
FC_w1 = tf.Variable(tf.truncated_normal([7*7*64, 512],stddev=0.1),name='FC_w1')
FC_b1 = tf.Variable(tf.truncated_normal([512],stddev=0.1),name='FC_b1')
FC_out1 = tf.nn.relu(tf.matmul(conv_out3_flat, FC_w1) + FC_b1)
#========build final fully connected layer=========
FC_w2 = tf.Variable(tf.truncated_normal([512, output_num_final],stddev=0.1), name = 'FC_w2')
FC_b2 = tf.Variable(tf.truncated_normal([output_num_final],stddev=0.1), name = 'FC_b2')
FC_out2 = tf.matmul(FC_out1, FC_w2) + FC_b2
...
#========build 1st fully connected layer===========
target_FC_w1 = tf.placeholder(dtype = tf.float32, shape = [7*7*64, 512], name='target_FC_w1')
target_FC_b1 = tf.placeholder(dtype = tf.float32, shape = [512], name='target_FC_b1')
FC_out1_target = tf.nn.relu(tf.matmul(conv_out3_target_flat, target_FC_w1) + target_FC_b1)
#========build final fully connected layer=========
target_FC_w2 = tf.placeholder(dtype = tf.float32, shape = [512, output_num_final], name = 'target_FC_w2')
target_FC_b2 = tf.placeholder(dtype = tf.float32, shape = [output_num_final], name = 'target_FC_b2')
FC_out2_target = tf.matmul(FC_out1_target, target_FC_w2) + target_FC_b2

output_Q = FC_out2
target_Q = FC_out2_target
...
update_counter = 0
Q5Saver(online_ConvW1,online_ConvB1,online_ConvW2,online_ConvB2,online_ConvW3, online_ConvB3,FC_w1,FC_b1,FC_w2,FC_b2,'P0',sess)
while update_counter < num_iteration:
    env.reset()
    rewardOneEpisode = 0
    for j in range(batch_size, Max_TimeStep):
        if update_counter % num_update_target == 0:
            ...
            target_FC_w1_feed = FC_w1.eval(session = sess)
            target_FC_b1_feed = FC_b1.eval(session = sess)
            target_FC_w2_feed = FC_w2.eval(session = sess)
            target_FC_b2_feed = FC_b2.eval(session = sess)
        ...
        feed = {x:stateBatch, action:actionBatch, r:rewardBatch, next_x:
            nextStateBatch, terminal: terminalBatch, target_ConvW1:target_ConvW1_feed,target_ConvB1:target_ConvB1_feed,target_ConvW2:target_ConvW2_feed,target_ConvB2:target_ConvB2_feed,target_ConvW3:target_ConvW3_feed,target_ConvB3:target_ConvB3_feed,target_FC_w1:target_FC_w1_feed, target_FC_b1:target_FC_b1_feed,target_FC_w2:target_FC_w2_feed,target_FC_b2: target_FC_b2_feed}
        ...
        if update_counter == int(num_iteration/3 * 1):
            Q5Saver(online_ConvW1,online_ConvB1,online_ConvW2,online_ConvB2, online_ConvW3, online_ConvB3,FC_w1,FC_b1,FC_w2,FC_b2,'P1',sess)
        elif update_counter == int(num_iteration/3 * 2):
            Q5Saver(online_ConvW1,online_ConvB1,online_ConvW2,online_ConvB2, online_ConvW3, online_ConvB3,FC_w1,FC_b1,FC_w2,FC_b2,'P2',sess)
...
Q5Saver(online_ConvW1,online_ConvB1,online_ConvW2,online_ConvB2,online_ConvW3, online_ConvB3,FC_w1,FC_b1,FC_w2,FC_b2,'P3',sess)